...
export class DiagnosticatorService {
    ...
    async getDeviceStatus(): Promise<DeviceUsbStatus> {
        if (!this.usbDetect) {
            this.usbDetect = (<any>window).require('usb-detection');
        }
            ...
            try {
                ...
                try {
                    ...
                    if (!devices || !devices.length) {
                        ...
                        ret.result = "Computer doesn't see Quest 2 plugged in at all";
                        ret.recommendation = `- Try a different USB cable or a different USB port on your computer.  There are a lot of bad USB-C cables out there, maybe try several.
- Sometimes rebooting the Quest 2 may resolve the problem.
- Try rebooting your computer.`;
                        ...
                        if (!foundAny) {
                            ...
                            ret.recommendation =
                                "Some sort of USB device from Oculus is detected, but it's none of the ones SideQuest knows about.  Sidequest MAY still work if Oculus has changed things in a firmware update, but it may not.  You can try rebooting your Quest 2.  Make sure you've installed any available SideQuest updates.";
                            ...
                            if (devMode && adb) {
                                ret.result = 'The Quest 2 is connected, developer mode is on and ADB device is present';
                                ret.recommendation = `SideQuest should be working.  Your computer sees the Quest 2 and it looks like developer mode is on.  An ADB device is also connected.
If SideQuest isn't working, you can try the following things:
- Make sure your virus scanner isn't blocking anything related to sidequest or 'adb'.
- Uninstall and reinstall SideQuest
- Try rebooting the Quest 2 and your computer
- if you've done all these things and are still having issues, you can reach out to SideQuest support.`;
                                ...
                                ret.result = "The Quest 2 is connected, developer mode is on, but can't find an ADB device";
                                ret.recommendation = `SideQuest MAY still work.  Your computer sees the Quest 2 and it looks like developer mode is on, but it isn't clear if the ADB device or driver is present.  
If SideQuest isn't working, you can try:
- Reboot the Quest 2
- Reboot your computer
- If you're on windows, try installing the Oculus software (which is currently the only way to manually install the USB drivers for the quest).`;
                                ret.result_status = UsbDiagResultStatus.Maybe;
                            } else if (adb) {
                                ret.result =
                                    "The Quest 2 is connected, developer mode is off, but there's an ADB device showing up?";
                                ret.recommendation =
                                    'Not sure how this has happened to you, but SideQuest MAY still work.  This is likely an unhandled device identifier in SideQuest.  Contact us and provide the log below so that we can add your scenario.';
                                ret.result_status = UsbDiagResultStatus.Maybe;
                            } else {
